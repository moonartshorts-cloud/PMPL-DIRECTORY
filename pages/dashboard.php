...
<button onclick="addEmp()">Save Record</button>
<button onclick="hide()" style="background:#fff; color:#333; border:1px solid #ddd">Cancel</button>
</div></div>
...
<input id="edit_code" placeholder="Emp Code">
<input id="edit_dept" placeholder="Department">
<input id="edit_desig" placeholder="Designation">
<input id="edit_site" placeholder="Site Name">
<input id="edit_mobile" placeholder="Mobile">
<input id="edit_email" placeholder="Email">
<button onclick="saveEdit()">Update Record</button>
<button onclick="hide()" style="background:#fff; color:#333; border:1px solid #ddd">Cancel</button>
</div></div>
...
<a href="../templates/employee_template.csv" style="display:block;margin-bottom:10px;color:#2563eb;text-decoration:none" download>⬇ Download Template</a>
...
<button type="button" onclick="hide()" style="background:#fff; color:#333; border:1px solid #ddd">Cancel</button>
</form>
</div></div>
...
function load(){
 ...
 .then(r=>r.json()).then(d=>{
  ...
  if(d.length === 0){
      b.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#999;padding:20px">No employees found</td></tr>';
      return;
  }
...
function addEmp(){
 ...
 })}).then(r=>r.json()).then(d=>{if(d.success){hide();load();en.value='';}});
}
...
function saveEdit(){
 ...
 })}).then(r=>r.json()).then(d=>{if(d.success){hide();load();} else {alert(d.msg)}});
}
...
function delEmp(id){
 ...
 .then(r=>r.json()).then(d=>{if(d.success) load(); else alert('Error deleting');});
}
